...
{
public:
    AP_Sonner_Backend(AP_Sonner &sonner);
    virtual ~AP_Sonner_Backend(void) {};
    ...
    bool waterdepth_ok(float waterdepth);
    uint32_t get_error_count() const { return _error_count; }
protected:
    // reference to frontend object
    AP_Sonner &_frontend;

    void _copy_to_frontend(uint8_t instance, float WaterDepth);
